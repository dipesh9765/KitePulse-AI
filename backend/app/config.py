...
PRIMARY_INSTRUMENTS_FILE = DATA_DIR / GenConsts.INSTRUMENTS_FILE_NAME
FALLBACK_INSTRUMENTS_FILE = APP_DIR.parent / GenConsts.INSTRUMENTS_FILE_NAME


def get_instruments_file_path() -> Path:
    if PRIMARY_INSTRUMENTS_FILE.exists():
        return PRIMARY_INSTRUMENTS_FILE
    return FALLBACK_INSTRUMENTS_FILE


def load_instruments_data(file_path: Path = None) -> List[Dict[str, Any]]:
    """Loads trading instruments metadata from external JSON configuration."""
    target_path = file_path or get_instruments_file_path()
    if target_path.exists():
        try:
            ...
            logger.error(f"Failed to load instruments config from {target_path}: {e}")
    return []
